...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        for loopCnt =  1:NumToProcess
            ...
            trackResults(channelNr).absoluteSample(loopCnt) =(ftell(fid))/dataAdaptCoeff*settings.dataSize;
            ...
            % interation
            dataAdaptCoeff = gather(dataAdaptCoeff);
            blksize = gather(blksize);
...
%             end
             ...
             % First mix to baseband
            iBasebandSignal = real(carrsig .* rawSignal);
            iBasebandSignal = gpuArray(single(iBasebandSignal));
            qBasebandSignal = imag(carrsig .* rawSignal);
            qBasebandSignal = gpuArray(single(qBasebandSignal));
          ...
          %  I_E = sum(earlyCode  .* iBasebandSignal);
           ...
           % Q_L = sum(lateCode   .* qBasebandSignal);
            ...
            I_E = sum(iBasebandSignal .* earlyCode);
            Q_E = sum(qBasebandSignal .* earlyCode);
            I_P = sum(iBasebandSignal .* promptCode);
            Q_P = sum(qBasebandSignal .* promptCode);
            I_L = sum(iBasebandSignal .* lateCode);
            Q_L = sum(qBasebandSignal .* lateCode);
            ...
            if (settings.pilotTRKflag == 2)
                % Correlation values for pilot BOC(1,1) spreading waveform
                p11_I_E = sum(iBasebandSignal .* p11_earlyCode);
                p11_Q_E = sum(qBasebandSignal .* p11_earlyCode);
                p11_I_P = sum(iBasebandSignal .* p11_promptCode);
                p11_Q_P = sum(qBasebandSignal .* p11_promptCode);
                p11_I_L = sum(iBasebandSignal .* p11_lateCode);
                p11_Q_L = sum(qBasebandSignal .* p11_lateCode);

                

                % Correlation values for pilot BOC(6,1) spreading waveform
                p61_I_E = sum(iBasebandSignal .* p61_earlyCode);
                p61_Q_E = sum(qBasebandSignal .* p61_earlyCode);
                p61_I_P = sum(iBasebandSignal .* p61_promptCode);
                p61_Q_P = sum(qBasebandSignal .* p61_promptCode);
                p61_I_L = sum(iBasebandSignal .* p61_lateCode);
                p61_Q_L = sum(qBasebandSignal .* p61_lateCode);
...
%             Q_L = gather(Q_L);
% 
%             p11_I_E = gather(p11_I_E);
%             p11_Q_E = gather(p11_Q_E);
%             p11_I_P = gather(p11_I_P);
%             p11_Q_P = gather(p11_Q_P);
%             p11_I_L = gather(p11_I_L);
%             p11_Q_L = gather(p11_Q_L);
% 
%             p61_I_E = gather(p61_I_E);
%             p61_Q_E = gather(p61_Q_E);
%             p61_I_P = gather(p61_I_P);
%             p61_Q_P = gather(p61_Q_P);
%             p61_I_L = gather(p61_I_L);
%             p61_Q_L = gather(p61_Q_L);
% 
%             p_I_E = gather(p_I_E);
%             p_I_P = gather(p_I_P);
%             p_I_L = gather(p_I_L);
...
%             iBasebandSignal = gather(iBasebandSignal);
%             qBasebandSignal = gather(qBasebandSignal);
...
%             codeError = gather(codeError);
%             codeNco = gather(codeNco);
%             carrError = gather(carrError);
%             carrNco = gather(carrNco);
